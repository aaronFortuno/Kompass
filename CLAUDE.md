...
El tema es presenta com a **reproductor de steps** (píndoles) — lliçó narrativa i exercicis intercalats, més una síntesi final. Vegeu ARCHITECTURE §18 (reproductor) i la memòria "Política de material didàctic".

1. **Esborrany de lliçó amb estructura d'steps:** llança un subagent general-purpose amb el PDF corresponent (`.gramatik/{nivell}/…`) + les directrius pedagògiques de la memòria (inclou catàleg d'exercicis). Output: `.drafts/{topicId}-lesson.md` amb separadors d'step explícits (`--- STEP: {id} ---`) i, per cada punt adient, suggerencies d'exercici en bloc identificat.
2. **Espera validació del desenvolupador** sobre l'esborrany abans de tocar codi. Pot reescriure lliurement.
3. Crea `src/data/topics/{nivell}/{num}-{slug}.json` seguint el schema `Topic` de DATA-MODEL §3.1:
   - Camp `steps[]` en lloc de `content[]`.
   - Cada step és `{ id?, blocks: [...] }` amb explanation/table/exercise barrejats.
   - L'últim step acostuma a ser la **síntesi** amb les taules tipus PDF.
4. **Exercicis**: pels que la lliçó requereix (quick-checks i avaluació final), esbossa els JSON a `src/data/exercises/{nivell}/{num}/`. El desenvolupador valida abans d'afegir-los al tema via blocs `exercise`.
...
7. Comprova visualment al browser: recorregut pas a pas, exercicis intercalats, avís final si queden exercicis pendents.
